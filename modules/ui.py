...
def create_toprow(is_img2img):
    ...
    with gr.Row(elem_id="toprow"):
        with gr.Column(scale=6, visible=False):
            with gr.Row():
...
#         with gr.Row().style(equal_height=False, visible=False):
...
#                     with gr.Accordion(open=False, label="Advanced", visible=False):
#                         with gr.Row():
#                             with gr.Column():
...
def create_ui(wrap_gradio_gpu_call):
    ...
    with gr.Blocks(analytics_enabled=False) as db_interface:
        with gr.Row(equal_height=True, visible=False):
            db_model_dir = gr.Dropdown(label='Model', choices=sorted(get_db_models()))
            db_half_model = gr.Checkbox(label="Half", value=False)
            db_load_params = gr.Button(value='Load Params')
    ...
    with gr.Blocks(analytics_enabled=False) as txt2img_interface:
        ...
        with gr.Row().style(equal_height=False):
            ...
            stl.change(fn=style_change, inputs=[stl], outputs=[bg, env])
    ...
    interfaces = [
        ...
        (db_interface, "dreambooth", "dreambooth"),
    ]
